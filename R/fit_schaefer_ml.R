...
fit_schaefer_ml <- function(C_obs, CPUE_obs, start_list = NULL, control = list(maxit = 1000)) {
  ...
  # 初期値（簡易）
  q0 <- 0.05
  B1_approx <- max(CPUE_obs[1] / q0, 1e-6)
...
fit_schaefer_ml_cpp <- function(C_obs, CPUE_obs, start_list = NULL, control = list(maxit = 1000),
  ...
  if (!exists("nll_schaefer_cpp", envir = .cpp_env, inherits = FALSE)) {
    ...
    Rcpp::sourceCpp(code = '
      ...
      // [[Rcpp::export]]
      NumericVector predict_B_schaefer_cpp(NumericVector C_obs, double r, double K, double B1, double guard) {
        int TT = C_obs.size();
        NumericVector B(TT);
        B[0] = B1;
        if (TT >= 2) {
      ...
      double nll_schaefer_cpp(NumericVector par, NumericVector C_obs, NumericVector CPUE_obs,
                              IntegerVector idx, double guard) {
  ...
  nll_fun <- function(par) {
    val <- .cpp_env$nll_schaefer_cpp(par, C_obs, CPUE_obs, idx_cpp, guard)
  ...
  B_hat <- .cpp_env$predict_B_schaefer_cpp(C_obs, est["r"], est["K"], est["B1"], guard)
