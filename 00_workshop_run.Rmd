...
# TRUEで実行（必要ならFALSEにする）
  ...
  if (do_sens_TT) {
    ...
    df_tt_true <- df_tt
    df_tt_true$Scenario <- "True"   # （凡例に True を出すため）
...
plot_mse_estimated <- function(Year, h_out, t2_out, K_ref, index_scale = 20, Bban_ratio = 0.3, Blim_ratio = 0.6) {
  ...
  hcr_lines <- data.frame(
    HCR = "Hockey (estimated Fmsy)",
    yint = c(Bban_ratio * K_ref, Blim_ratio * K_ref)
  )
  bio_q3 <- as.numeric(quantile(df_bio_hat$Biomass, probs = 0.75, na.rm = TRUE))
  bio_iqr <- IQR(df_bio_hat$Biomass, na.rm = TRUE)
  bio_upper <- bio_q3 + 1.5 * bio_iqr
  index_upper <- max(df_bio_hat$Index, na.rm = TRUE) * 1.1
  y_upper <- max(bio_upper, index_upper, hcr_lines$yint)
  ...
  p_bio_hat <- ggplot(df_bio_hat, aes(x = Year)) +
    ...
    geom_line(aes(y = Index, col = "Index*20")) +
    geom_point(aes(y = Index, col = "Index*20")) +
    geom_hline(data = hcr_lines, aes(yintercept = yint), linetype = "dashed", color = "red") +
    scale_x_continuous(expand = c(0, 0),
                       limits = c(min(Year) - 0.5, max(Year) + 0.5),
                       breaks = seq(min(Year), max(Year), 5)) +
    scale_color_manual(values = c("Index*20" = "darkorange"), name = NULL) +
...
plot_mse_estimated(Year, h_out, t2_out, K_ref = K, Bban_ratio = 0.3, Blim_ratio = 0.6)
