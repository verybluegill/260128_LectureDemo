...
mse_sum_h$HCR <- "Hockey (estimated)"
...
plot_mse_2 <- function(Year, h_out, t2_out, index_scale = 20) {
  ...
  df_bio <- rbind(
    data.frame(Year = Year, Biomass = h_out$B_true,  Index = h_out$I_obs  * index_scale, HCR = "Hockey (estimated)"),
  ...
  df_catch <- rbind(
    data.frame(Year = Year, Catch = h_out$C_true,  HCR = "Hockey (estimated)"),
